...
<x-admin.table 
    datefilter="true" 
    order="true" 
    addbutton="{{ route('admin.coupons.create') }}" 
    deletebutton="{{ route('admin.coupons.deleteAll') }}" 
    :searchArray="[
        'identity' => [
            'input_type' => 'text' , 
            'input_name' => awtTrans('رقم الكوبون') , 
        ],
    ]" 
>
    ...
    <script>
        ...
        $(document).on('click' , '.change-coupon-status', function () {
            $.ajax({
                ...
                data: {id : $(this).data('id') , status : $(this).data('status') , expire_date :$(this).data('date')},
                dataType: "json",
                success:  (response)=> {
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        $.each(xhr.responseJSON.errors, function(key,value) {
                            $('.notify-form input[name='+key+']').addClass('border-danger')
                            $('.notify-form input[name='+key+']').after(`<span class="mt-5 text-danger">${value}</span>`);
